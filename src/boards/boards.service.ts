...
export class BoardsService {
  ...
  async create(createBoardDto: Prisma.BoardsCreateManyInput) {
    return await this.prisma.boards.create({ data: createBoardDto });
  }
  ...
  async findOne(id: number) {
    return await this.prisma.boards.findUnique({ where: { id } });
  }
  ...
  async remove(id: number): Promise<Boards> {
    return await this.prisma.boards.delete({ where: { id } });
  }
}
